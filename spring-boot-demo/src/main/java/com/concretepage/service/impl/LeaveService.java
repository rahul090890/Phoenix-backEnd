...
public class LeaveService implements ILeaveService {
	...
	public EmployeeLeave applyLeave(EmployeeLeave leave) throws HRException {
		...
		try {
		emailSender.prepareAndSend(leave.getEmployee().getEmailId(), "LeaveApplied", prepareDynamicFieldsForLeaveEmails(leave), "leaveApplied");
		} catch(Exception e) {
			log.error("Error in sending email " + e);
		}
		leaveDao.applyLeave(leave);
		leaveBalanceDao.incrementLeaveBalance(leave.getEmployee().getEmployeeId(), DateUtils.getCurrentYear(), leave.getLeaveType(), leave.getNoOfDays(), 0);
		return leave;
	}
	...
	public EmployeeLeave approveLeave(int leaveId,  String comments) throws HRException {
		...
		if(leave.getLeaveStatus().equals(LEAVESTATUS.PENDING_APPROVAL.name())) {
			leaveBalanceDao.incrementLeaveBalance(leave.getEmployee().getEmployeeId(), DateUtils.getCurrentYear(), leave.getLeaveType(), -leave.getNoOfDays(), -leave.getNoOfDays());
			//rejected to approval - only decrease eligibility
		} else if (leave.getLeaveStatus().equals(LEAVESTATUS.REJECTED.name())){
			leaveBalanceDao.incrementLeaveBalance(leave.getEmployee().getEmployeeId(), DateUtils.getCurrentYear(), leave.getLeaveType(), 0, -leave.getNoOfDays());
		}
		try {
		emailSender.prepareAndSend(leave.getEmployee().getEmailId(), "LeaveApplied", prepareDynamicFieldsForLeaveEmails(leave), "leaveApproved");
		} catch(Exception e) {
			log.error("Error in sending email " + e);
		}
	...
	public EmployeeLeave rejectLeave(int leaveId,  String comments) throws HRException {
		EmployeeLeave leave = leaveDao.getEmployeeLeave(leaveId);
		// can only reject, if it is pending approval. Approved leaves can not be rejected
		...
		emailSender.prepareAndSend(leave.getEmployee().getEmailId(), "LeaveApplied", prepareDynamicFieldsForLeaveEmails(leave), "leaveRejected");
		leave.setComments(comments);
	...
	/**private void sendEmailforLeaveApplied(EmployeeLeave leave) {
		...
		dynamicFields.put("managerName", leave.getEmployee().getManager().getFirstName() + " " + leave.getEmployee().getManager().getLastName());
		
		emailSender.prepareAndSend(leave.getEmployee().getEmailId(), "LeaveApplied", dynamicFields, "leaveApplied");
	}
	**/
	...
	private Boolean hasEmployeeEnoughBalance(EmployeeLeave employeeLeave) {
		...
		List<EmployeeLeaveBalance> leaveBalance = leaveBalanceDao.getLeaveBalance(employeeLeave.getEmployee().getEmployeeId(), leaveToDate.getYear());
